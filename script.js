...
function genererNombreEntier(max){
  ...
  // Math.random genère un nb entre [0;1[
}
btn.addEventListener('click', () => {
  do {
    nbRandom = genererNombreEntier(citations.length);
  } while (nbRandom == lastCitation)
  citation.textContent = citations[nbRandom][0];
  author.textContent = citations[nbRandom][1];
  lastCitation = nbRandom;
})
